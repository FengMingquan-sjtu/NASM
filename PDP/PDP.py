...
class ZermeloSys:
    '''Solve Zermelo problem '''
    def __init__(self, project_name="my optimal control system"):
        self.project_name = project_name
        self.control = SX.sym('Beta')
        self.control_lb = [0]
        self.control_ub = [np.pi]
        self.time_step = SX.sym('dt')
        self.time_step_lb = [0]
        self.time_step_ub = [10]

        self.x, self.y = SX.sym('X'),SX.sym('Y')
        self.state_lb = [-10]
        self.state_ub = [10]
        

    def solver(self, ini_state, horizon, print_level=0):
    
        if type(ini_state) == numpy.ndarray:
            ini_state = ini_state.flatten().tolist()
        x2, y2 = [ini_state[0]], [ini_state[1]]
        V = ini_state[2]
        A,B = ini_state[3:5] #u(x,y) = Ax+By
        C,D =  ini_state[5:7] #v(x,y) = Cx+Dy
        
        
        # Start with an empty NLP
        w = [] #control variables list
        w0 = []
        lbw = []
        ubw = []
        s_w = [] #state variables list
        s_w0 = []
        s_lbw = []
        s_ubw = []
        J = 0
        ...
        dt = SX.sym('dt_0')
        w += [dt]
        lbw += self.time_step_lb
        ubw += self.time_step_ub
        w0 += [0.5 * (self.time_step_lb[0]+self.time_step_ub[0])]


        #print("Length of horizon", len(w))
        #Formulate NLP
        ...
        for k in range(horizon+1):
            ...
            Xnext = Xk + dt * (V*cos(Betak) + A*Xk + B*Yk)
            Ynext = Yk + dt * (V*sin(Betak) + C*Xk + D*Yk)

            # New NLP variable for state at end of interval
            Xk, Yk = SX.sym('X_' + str(k + 1)), SX.sym('Y_' + str(k + 1))
            s_w += [Xk, Yk]
            if k == horizon: #final state
                s_lbw += x2 
                s_lbw += y2 
                s_ubw += x2
                s_ubw += y2
                ...
                s_lbw += [self.state_lb[0],self.state_lb[0]]
                s_ubw += [self.state_ub[0],self.state_ub[0]]
                s_w0 += [(self.state_lb[0]+self.state_ub[0])/2, (self.state_lb[0]+self.state_ub[0])/2 ]
        ...
        dt = w_opt[0]
        control_traj_opt = w_opt[1: 1+horizon+1]
        cost = horizon * dt

        # output
        opt_sol = {
                   "control_traj_opt": control_traj_opt,
                   "xy" : w_opt[1+horizon+1:],
                   "dt": dt,
                   "horizon": horizon,
                   "cost": sol['f'].full()}
    ...
    def ana_solver(self, ini_state, horizon):
        if type(ini_state) == numpy.ndarray:
            ini_state = ini_state.flatten().tolist()
        x2, y2 = [ini_state[0]], [ini_state[1]]
        V = ini_state[2]
        ...
        def derivatives(t, vars): 
            beta, x, y = vars
            #return [dbeta/dt, dx/dt, dy/dt]
            return np.array([
                C*numpy.sin(beta)**2 + (A-D)*numpy.sin(beta)*numpy.cos(beta) - B*numpy.cos(beta)**2,
                V*numpy.cos(beta) + A*x +B*y,
                V*numpy.sin(beta) + C*x +D*y,
            ])
            
        
        def f(x): #nonlinear equation to solve
            # x[0] = beta_init, x[1]=T_final
            #solve_ivp(fun, t_span, y0, method='RK45',..)
            sol = solve_ivp(derivatives, (0, x[1]), np.array([x[0], 0, 0]), method='RK23',)
            x_T_final, y_T_final = sol['y'][1:,-1] 
            return np.array([x_T_final- x2,  y_T_final-y2]).squeeze()
        
        beta_init, T_final = fsolve(f, [0,1]) #fsolve(func, x0), x0 is initial guess
        sol = solve_ivp(derivatives, (0, T_final), np.array([beta_init, 0, 0]), method='RK23')
        t, beta_t = sol['t'],  sol['y'][0]
        t_new = np.arange(0, horizon+1) * T_final / horizon
        beta_t_new = np.interp(t_new, t, beta_t)  
        return beta_t_new



class BrachistochroneSys:
    '''Solve Brachistochrone problem '''
    def __init__(self, project_name="my optimal control system"):
        self.project_name = project_name
        self.control = SX.sym('U')
        self.n_control = self.control.numel()
        self.control_lb = self.n_control * [-10]
        self.control_ub = self.n_control * [10]

        

    def solver(self, ini_state, horizon, dx, print_level=0):
        self.control = SX.sym('U')

        if type(ini_state) == numpy.ndarray:
            ini_state = ini_state.flatten().tolist()
        left_state, right_state = [ini_state[0]], [ini_state[1]]
        assert left_state[0] > right_state[0]
        self.control_ub = left_state
        ...
        for k in range(1, horizon+1):
            # finite difference dUk/dx, d^2Uk/dx^2
        
            if k==horizon:
                dUk = (1.5 * w[k] - 2 * w[k-1] + 0.5*w[k-2]) / dx
                ddUk = (2 * w[k] - 5 * w[k - 1] + 4 * w[k - 2] - w[k - 3]) / dx ** 2
            else:
                dUk = (w[k] - w[k-1]) /(dx)
                ddUk = (w[k + 1] - 2 * w[k] + w[k - 1]) / dx ** 2
            Uk = w[k]
            Ck = sqrt((1 + dUk**2 ) / (left_state[0] - Uk)) + 0.05*dUk**2 + 0.0005*ddUk**2  #+ 0.1*Uk*Uk
            J += Ck
        ...
        # take the optimal control and state
        control_traj_opt = w_opt
        x_idx = numpy.array([k for k in range(horizon + 1)])*dx
    ...
    def get_instance(self,ini_state, x_bound):
        left_state, right_state = ini_state
        def f(x):
            return [left_state - x[0]*(1-np.cos(x[1])) - right_state, x[0]*(x[1] - np.sin(x[1]))-x_bound]
        k, Theta = fsolve(f, [0,0])
        
        opt_cost = np.sqrt(2*k) * Theta
         
        return opt_cost, k, Theta

    def ana_solver(self, ini_state, horizon, dx, k, Theta):
        """ u(theta) = ini_state[0] - k(1-cos(theta))
            x(theta) = 0 + k(theta - sin(theta))
            u(0), u(Theta) = ini_state[0], ini_state[1]
            x(0), x(Theta) = 0, horizon *dx
        """
        theta = numpy.linspace(0, Theta, num=10000)
        u_theta = ini_state[0] - k*(1-np.cos(theta))
        x_theta = k* (theta - np.sin(theta))
        x = np.arange(0, horizon+1) * dx
        u_x = np.interp(x, x_theta, u_theta)  #u(x)
        return u_x
...
class DAESys:
    ...
    def solver(self, ini_state, horizon, u, print_level=0):
        ...
        prob = {'f': J, 'x': vertcat(*w), 'g': vertcat(*g)}
        solver = nlpsol('solver', 'ipopt', prob, opts)
        #solver = nlpsol('solver', 'scpgen', prob, opts)
        # Solve the NLP
        sol = solver(x0=w0, lbx=lbw, ubx=ubw, lbg=lbg, ubg=ubg)
...
class OCSys:
    ...
    def ocSolver(self, ini_state, horizon, auxvar_value=1, print_level=0, costate_option=0):
        ...
        # Create an NLP solver and solve it
        opts = {'ipopt.print_level': print_level, 'ipopt.sb': 'yes', 'print_time': print_level}
        #opts = {'verbose':False, 'qpsol':'qpoases', 'hessian_approximation': 'gauss-newton','print_header': False, 'print_time': print_level, 'qpsol_options':{'printLevel':"none", "error_on_fail":False}}
        prob = {'f': J, 'x': vertcat(*w), 'g': vertcat(*g)}
        solver = nlpsol('solver', 'ipopt', prob, opts)
        #solver = nlpsol('solver', 'scpgen', prob, opts)
        # Solve the NLP
